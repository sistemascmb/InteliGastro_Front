...
const cargarSalas = async () => {
  ...
  const cargarProcedimientos = async () => {
        try {
          const procedimientos = await appointmentsService.getAll_Cons_Agendado();
          ...
          const listaAgendasProcedimientos = await Promise.all(
              procedimientos.data.map(async (procedimientoDat) => {
                try {
                  ...
                  const statusIdRaw = parseInt(procedimientoDat.status, 10);
                  const statusId = Number.isFinite(statusIdRaw) ? statusIdRaw : 0;
                  const estadoAgenda = statusId > 0
                    ? await centrosService.getSystemParameterId(statusId)
                    : { data: { value1: 'Estado no especificado' } };
                  
                  
                  const medicoTratDatos = procedimientoDat.personalId
                    ? await staffService.getById(procedimientoDat.personalId)
                    : { data: { nombres: '', apellidos: '' } };
                  
                  const seguroDatos = await segurosService.getById(procedimientoDat.insuranceId);
                  const typeOfPatientIdRaw = parseInt(procedimientoDat.typeOfPatient, 10);

                  const tipoAtencion =
                     await centrosService.getSystemParameterId(procedimientoDat.typeofAppointment)
                    

                  const tipoProcedIdRaw = parseInt(procedimientoDat.tipoProcedimientoId, 10);
                  const tipoProcedId = Number.isFinite(tipoProcedIdRaw) ? tipoProcedIdRaw : 0;
                  const tipoProced = tipoProcedId > 0
                    ? await centrosService.getSystemParameterId(tipoProcedId)
                    : { data: { value1: 'Tipo de procedimiento no especificado' } };
                  ...
                  const cie10Pairs = await Promise.all((listaCie10 || []).map(async (dx) => {
                    try {
                      const rawId = parseInt(dx?.cie10id, 10);
                      const hasValidId = Number.isFinite(rawId) && rawId > 0;
                      if (!hasValidId) {
                        const code = String(dx?.cie10id || '').trim();
                        const desc = dx?.description || '';
                        const out = [code, desc].filter(Boolean).join(':');
                        return out || 'CIE-10 no especificado';
                      }
                      const cie = await cie10Service.getById(dx.cie10id);
                      const c = Array.isArray(cie) ? null : (cie?.data || {});
                      const code = c?.codigo || c?.code || String(dx.cie10id || '');
                      const desc = c?.descripcion || c?.description || dx?.description || '';
                      const out = [code, desc].filter(Boolean).join(':');
                      return out || 'CIE-10 no especificado';
                    } catch {
                      const code = String(dx?.cie10id || '').trim();
    ...
    if (file && uploadForm.tipoArchivo) {
      try {
        ...
        try {
          ...
          cargarArchivos(selectedProcedimiento.codigo);
  ...
  const getArchivoTipoColor = (mime) => {
    ...
    if (mime === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') return 'info';
    return 'default';
  };
    ...
    <Container maxWidth="lg" sx={{ py: 1, px: 2, maxWidth: '100% !important' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
        ...
        <Paper sx={{ flex: 1, boxShadow: 3, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <SectionHeader title={`Lista de Citas Agendadas (${procedimientosFiltrados.length})`} />
              ...
              <Table stickyHeader>
                <TableHead>
                  <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                    ...
                    <TableCell><strong>Cie10</strong></TableCell>
                          ...
                          <Box>
                            <Typography variant="body2" color="primary">
                             ...
                             {!proc.gastroenterologo ? (
                               <Chip label="No especificado" color="warning" size="small" variant="outlined" sx={{ ml: 0.5 }} />
                             ) : null}
                            </Typography>
                          </Box>
                        </TableCell>
