...
const cargarSalas = async () => {
  ...
  const handleBuscarProcedimientos = async () => {
    try {
      const idVal = String(filters.medicalscheduleid || '').trim();
      const namesVal = String(filters.names || '').trim();
      const lastNamesVal = String(filters.lastNames || '').trim();
      const dniVal = String(filters.dni || '').trim();

      const hasAny = idVal || namesVal || lastNamesVal || dniVal;
      if (!hasAny) {
        setProcedimientosAgendados([]);
        setHasSearched(false);
        setSnackbar({ open: true, message: 'Ingrese al menos un criterio de búsqueda', severity: 'warning' });
        setError('');
        return;
      }

      const params = {
        medicalscheduleid: idVal ? parseInt(idVal, 10) : undefined,
        names: namesVal || undefined,
        lastNames: lastNamesVal || undefined,
        dni: dniVal || undefined
      };

      const resultado = await appointmentsService.getAll_Proc_Buscados(params);
      const base = Array.isArray(resultado?.data) ? resultado.data : [];
      const filtrado = base.filter((p) => !p.isDeleted);// && p.typeOfAttention === 2 && p.status === 10065);

      const listaAgendasProcedimientos = await Promise.all(
        filtrado.map(async (procedimientoDat) => {
          try {
            const examenDatos = await estudiosService.getById(procedimientoDat.studiesId);
            const centroDatos = await centrosService.getById(procedimientoDat.centroId);
            const pacienteDatos = await patientsService.getById(procedimientoDat.pacientId);
      ...
      setProcedimientosAgendados(listaAgendasProcedimientos);
      setHasSearched(true);
      setError('');
    } catch (error) {
      setProcedimientosAgendados([]);
  ...
  const procedimientosFiltrados = procedimientosAgendados;
  ...
  const handleConfirmPacienteInicio = async(e) => {
    ...
    await handleBuscarProcedimientos();
    ...
    <Container maxWidth="lg" sx={{ py: 1, px: 2, maxWidth: '100% !important' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
        ...
        <Paper sx={{ p: 3, mb: 3, minHeight: '20vh', boxShadow: 3 }}>
          ...
          <FieldRow>
            <ResponsiveField label="Código Estudio (ID)" sx={{ flex: 1 }}>
              <TextField
                fullWidth
                placeholder="Ej. 12345"
                value={filters.medicalscheduleid}
                onChange={(e) => handleFilterChange('medicalscheduleid', e.target.value)}
                size="small"
                InputProps={{
                  startAdornment: (
            ...
            <ResponsiveField label="Nombres" sx={{ flex: 1 }}>
              <TextField
                fullWidth
            ...
            <ResponsiveField label="Apellidos" sx={{ flex: 1 }}>
              <TextField
                fullWidth
                placeholder="Ej. Pérez Gómez"
                value={filters.lastNames}
                onChange={(e) => handleFilterChange('lastNames', e.target.value)}
                size="small"
                sx={{
                  '& .MuiOutlinedInput-root': {
                    backgroundColor: '#f8f9fa',
            ...
            <ResponsiveField label="DNI" sx={{ flex: 1 }}>
              <TextField
                fullWidth
        ...
        <Paper sx={{ flex: 1, boxShadow: 3, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <SectionHeader title={`Lista de Procedimientos (${procedimientosFiltrados.length})`} />
                    ...
                    <TableRow>
                      <TableCell colSpan={8} align="center" sx={{ py: 4 }}>
                        <Typography variant="body1" color="text.secondary">
                          {hasSearched ? 'No se encontraron resultados' : 'No se ha realizado búsqueda'}
                        </Typography>
                      </TableCell>
                    </TableRow>
                  ) : (
      ...
      >
        <DialogTitle component="div" sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#2184be', color: 'white' }}>
